...
std::string UdpDoubleSernder::lastSockErr_() {
#if defined(_WIN32)
...
#if defined(_WIN32)
struct UdpDoubleSernder::WinsockRAII {
    WinsockRAII() {
        WSADATA w{};
        if (WSAStartup(MAKEWORD(2,2), &w) != 0) {
            throw std::runtime_error("WSAStartup failed: " + UdpDoubleSernder::lastSockErr_());
        }
    }
...
// ---------- endian helpers ----------
bool UdpDoubleSernder::isLittleEndian_() {
    uint16_t one = 1;
    return *reinterpret_cast<uint8_t*>(&one) == 1;
}

uint64_t UdpDoubleSernder::bswap64_(uint64_t x) {
#if defined(_MSC_VER)
...
uint64_t UdpDoubleSernder::hostToBE64_(uint64_t v) {
    return isLittleEndian_() ? bswap64_(v) : v;
}

void UdpDoubleSernder::writeBE16_(uint8_t* dst, uint16_t v) {
    uint16_t be = htons(v);
    std::memcpy(dst, &be, sizeof(be));
}

void UdpDoubleSernder::writeBE32u_(uint8_t* dst, uint32_t u) {
    uint32_t be = htonl(u);
    std::memcpy(dst, &be, sizeof(be));
}

void UdpDoubleSernder::writeBE32_fromBits_(uint8_t* dst, int32_t s) {
    uint32_t u = 0;
...
void UdpDoubleSernder::writeBE64_fromBits_(uint8_t* dst, int64_t s) {
    uint64_t u = 0;
    static_assert(sizeof(u) == sizeof(s), "width mismatch");
    std::memcpy(&u, &s, sizeof(u));   // preserve bits
...
void UdpDoubleSernder::writeDoubleBE_(uint8_t* dst, double d) {
...
// ---------- misc ----------
int UdpDoubleSernder::desiredFamily_(IpMode m) {
    switch (m) {
...
bool UdpDoubleSernder::isOpen_() const {
#if defined(_WIN32)
...
void UdpDoubleSernder::closeSock_() {
#if defined(_WIN32)
...
void UdpDoubleSernder::initTimestampOffset_() {
...
int64_t UdpDoubleSernder::monotonicNowNanosNonNegative_() const {
    using namespace std::chrono;
    const int64_t now = (int64_t)duration_cast<nanoseconds>(
        steady_clock::now().time_since_epoch()).count();
...
// ---------- ctor/dtor ----------
UdpDoubleSernder::UdpDoubleSernder(const std::string& remoteHost,
                                   uint16_t remotePort,
                                   uint16_t localPort,
                                   int maxDoubles,
                                   bool connectUdp,
...
UdpDoubleSernder::~UdpDoubleSernder() {
    close();
#if defined(_WIN32)
    delete winsock_;
    winsock_ = nullptr;
...
UdpDoubleSernder::UdpDoubleSernder(UdpDoubleSernder&& other) noexcept { moveFrom_(std::move(other)); }

UdpDoubleSernder& UdpDoubleSernder::operator=(UdpDoubleSernder&& other) noexcept {
    if (this != &other) {
        close();
#if defined(_WIN32)
        delete winsock_;
...
void UdpDoubleSernder::moveFrom_(UdpDoubleSernder&& o) noexcept {
...
// ---------- socket setup ----------
bool UdpDoubleSernder::bindLocal_(decltype(sock_) s, int family, uint16_t localPort) {
...
void UdpDoubleSernder::createAndConfigureSocket_(uint16_t localPort, int requestedSndBuf) {
    addrinfo hints{};
...
// ---------- public API ----------
int UdpDoubleSernder::getMaxDoubles() const { return maxDoubles_; }

int UdpDoubleSernder::getSendBufferBytes() const {
    if (!isOpen_()) return 0;
...
void UdpDoubleSernder::setUnicastHopLimit(int hops) {
    if (!isOpen_()) return;
...
void UdpDoubleSernder::setMulticastHopLimit(int hops) {
    if (!isOpen_()) return;
    hops = std::max(0, std::min(hops, 255));
    if (family_ == AF_INET) {
...
void UdpDoubleSernder::setMulticastLoop(bool enable) {
    if (!isOpen_()) return;
    if (family_ == AF_INET) {
...
void UdpDoubleSernder::setMulticastInterfaceIPv4(uint32_t ifAddrNbo) {
...
void UdpDoubleSernder::setMulticastInterfaceIPv6(unsigned int ifIndex) {
    if (!isOpen_() || family_ != AF_INET6) return;
    setsockopt(sock_, IPPROTO_IPV6, IPV6_MULTICAST_IF, (char*)&ifIndex, (socklen_t)sizeof(ifIndex));
}

size_t UdpDoubleSernder::sendAutoSeq(const double* data, int count) {
    return sendWithSeq(data, count, seq_++);
}

size_t UdpDoubleSernder::sendWithSeq(const double* data, int count, int32_t seq, int64_t timestampNanos) {
...
void UdpDoubleSernder::close() { closeSock_(); }
